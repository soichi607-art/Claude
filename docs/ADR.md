...
## ADR-005: HTMX 本体を同梱せず、最小の自前 JS を使用（2026-09-09）
- 状況: 開発環境から CDN（jsdelivr/unpkg）へ到達できず、HTMX を取得できなかった。オフライン動作と「通信費以外0円」の要件から外部 CDN 依存は避けたい。
- 決定: `app/static/app.js` に部分更新ポーリング（`data-poll`）、確認ダイアログ、コピー、range 表示だけを実装（約 60 行）。テンプレートはサーバーサイド Jinja2 のまま。
- 影響: HTMX の高度な機能は使わない。将来 HTMX を同梱する場合は BSD ライセンスのファイルを `app/static/` に置くだけで置換可能。
