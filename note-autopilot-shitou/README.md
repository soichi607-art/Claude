...
外さずに、**失敗率を下げ、失敗しても当日中に取り返す**構成にしています。

| 対策 | 何を防ぐか |
|---|---|
| **毎日 08:00 の疎通確認** | Cookie 失効を「投稿日の朝」に見つける。週1回では失効したまま投稿日を迎える |
| **09:00 に記事を事前生成** | 生成の失敗と公開の失敗を切り離す。公開まで8時間の余裕ができる |
...
| **失敗時は必ず通知** | `.env` の `NOTIFY_WEBHOOK_URL` があれば送信。無ければ `logs/` に記録 |

**リトライしないもの**: 終了コード2(緊急停止)。これはリトライしてはいけない失敗です。

**残るリスク**(人の手が必要):

1. Cookie が失効したまま復旧されない → 08:00 の通知に対応してください
2. note 側の仕様変更 → 疎通確認が検知します。コードの修正が必要です
3. Gemini 無料枠の枯渇 → 生成が3回とも失敗します。17:00 にも再試行されます
4. 品質ゲートを6回とも通らない → `config.b.yaml` の `min_score` を 75 へ下げる(品質との引き換え)
...
| 火木土 09:00 | `prepare-b` | sync → collect → analyze → generate。記事を用意 |
